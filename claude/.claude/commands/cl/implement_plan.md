...
You are tasked with implementing an approved technical plan from the codelayer vault (`~/dev/codelayer-vault/{project}/`). These plans contain phases with specific changes and success criteria.
...
```
Use this as `{project}` in vault paths: `~/dev/codelayer-vault/{project}/TASKNAME/`
